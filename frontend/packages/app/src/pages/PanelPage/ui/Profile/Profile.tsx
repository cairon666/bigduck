    ...
    return (
        <div ref={ref} className={classNames('relative hover:bg-gray-20', isOpen ? 'bg-gray-20' : '')}>
            <button
                type="button"
                onClick={onOpen}
                className="flex  cursor-pointer items-center  gap-1 border-transparent p-0 px-2 py-2"
            >
                <UserAvatar />
            ...
            >
                <Card className="absolute -right-1 top-full z-20 flex w-72 flex-col !rounded-t-none shadow-md">
                    ...
                    {list.map((item) => {
                        ...
                        border-b-transparent px-4 py-2 font-light duration-75 ease-linear hover:border-yellow-500 
                        hover:bg-gray-20`,
                                )}
                            >
