...
const registerScheme = object({
    ...
    username: string().required('Никнейм обязательный параметр').min(4, 'Никнейм должен содержать от 4 символов'),
    first_name: string().required('Имя обязательный параметр').min(4, 'Имя должно содержать от 4 символов'),
    second_name: string().required('Фамилия обязательный параметр').min(4, 'Фамилия должна содержать от 4 символов'),
    email: string().required('Почта обязательный параметр').email('Неверный формат почты'),
});
...
export const useRegisterForm = () => {
    ...
    const [alreadyExist, setAlreadyExist] = useState<Partial<Record<'username' | 'email' | 'login', boolean>>>({});
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            ...
            navigate(`/auth/login?login=${data.login}&password=${data.password}`);
        } catch (e) {
